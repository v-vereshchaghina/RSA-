...
def brute_force_rsa(ciphertext, public_key):
    ...
    letters = string.ascii_lowercase  # Латинский алфавит a-z
    ...
    for length in range(1, 6): # Максимальная длина слова
        for word in itertools.product(letters, repeat=length):
            ...
            if rsa_encrypt(guess_number, public_key) == ciphertext: # нашли(подобрали) слово
                end_time = time.time()
                return guess_word, end_time - start_time # Слово и время
...
public_key = private_key.public_key() # Ключ
length = random.randint(1, 4)  # Длина от 1 до 4 букв
# message = input('input your text: ') # Ввод слова
message = ''.join(random.choice(string.ascii_lowercase) for _ in range(length)) # Случайное слово
